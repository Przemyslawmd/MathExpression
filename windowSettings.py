...
class WindowSettings(QDialog):
    ...
    def __init__(self, parent, settings):
        ...
        row = 1
        layout_grid.addWidget(QLabel("X Grid"), row, 1)
        self.check_x_grid = QCheckBox()
        self.check_x_grid.setChecked(settings.x_grid)
        layout_grid.addWidget(self.check_x_grid, row, 2, alignment=QtCore.Qt.AlignRight)

        row += 1
        layout_grid.addWidget(QLabel("Y Grid"), row, 1)
        self.check_y_grid = QCheckBox()
        self.check_y_grid.setChecked(settings.y_grid)
        layout_grid.addWidget(self.check_y_grid, row, 2, alignment=QtCore.Qt.AlignRight)
