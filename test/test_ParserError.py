...
class TestParserError(TestCase):
    ...
    def test_parser_bracket_1(self):
        ...
        self.assertEqual(errors[0], Message[Error.PARSER_BRACKET])
    ...
    def test_parser_symbol_1(self):
        Parser("12y + 4").parse()
        errors = ErrorStorage.get_errors()
        self.assertEqual(errors[0], Message[Error.PARSER_SYMBOL] + ": y")


    def test_parser_symbol_2(self):
        Parser("tg45 * cor30 - 3").parse()
        errors = ErrorStorage.get_errors()
        self.assertEqual(errors[0], Message[Error.PARSER_SYMBOL] + ": c")


    def test_parser_negative_1(self):
        Parser("(-*5 + x)").parse()
        errors = ErrorStorage.get_errors()
        self.assertEqual(errors[0], Message[Error.PARSER_NEGATIVE_SYMBOL])
    ...
    def test_parser_angle_bracket_1(self):
        Parser("(5x + x)(<5> + <3)").parse()
        errors = ErrorStorage.get_errors()
        self.assertEqual(errors[0], Message[Error.PARSER_BRACKET_ANGLE])
